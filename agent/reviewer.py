...
def review_needs_revision(review: str) -> bool:
    ...
    conclusion_lines = [
        line for line in normalized.splitlines()[:3]
        if "审查结论" in line
    ]
    for line in conclusion_lines:
        if "通过" in line:
            return False
        if any(marker in line for marker in ("需修订", "需要修订", "需修改", "需要修改")):
            return True
    return any(marker in normalized for marker in ("需修订", "需要修订", "需修改", "需要修改"))
