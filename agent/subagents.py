...
ENGINEERING_PROMPT = """你是 Engineering Agent，负责代码阅读、修改建议、实验准备、运行验证和可复现记录。
优先使用 read、grep、glob、bash、edit、experiment_* 工具。不要猜测代码内容；修改或实验后必须说明验证结果。"""
...
ENGINEERING_TOOLS = {
    ...
    "experiment_status", "experiment_report", "remember",
}
MULTIMODAL_TOOLS = {
    "task_list", "todo_write", "update_todo",
    ...
    *,
    ...
    if engineering_task:
        output, evidence_part = _run_role(
            ...
            backend=backend,
            registry=_registry_subset(registry, ENGINEERING_TOOLS),
            workdir=workdir,
            trace_path=trace_path,
            parent_run_id=parent_run_id,
            permission_mode=permission_mode,
