...
3. **下载 PDF、补充材料和代码**
   - 用 `bash` 下载 PDF 和 supplement，保存到清晰目录，例如 `literature/<topic>/pdfs/`；所有项目产物使用相对路径。
   - 如果用户要求复现导向调研，优先收集 GitHub 仓库链接；需要本地分析时再 clone。
   - 不要下载无关大文件；下载失败时记录失败原因和源链接。

4. **选择 PDF 解析工具**
   - 调用 `pdf_extract_text`，由工具先检查 GPU；显存满足阈值时使用 `marker`，否则降级到 `markitdown`，两者异常时才使用 pypdf 兜底。
   - MarkItDown 路径同时使用 PyMuPDF 保存嵌入图片和页面渲染图，输出 `paper.md`、`image_manifest.json` 与 `images/`。
   - 解析后用 `read` 阅读 markdown，不要凭文件名猜内容；若两种主解析器都不可用，要在报告中明确说明降级原因。

5. **论文图表与多模态补充**
   - 当用户提供论文图、方法框架图、实验曲线、结果表格截图，或 PDF 解析结果缺失关键 figure/table 信息时，配合 `paper-figure-reader` skill。
   - 先调用 `paper_figure_analyze` 对解析产物中的图片进行分析；工具会按本 skill 的证据规则调用视觉后端，提取图中可见文字、模块、坐标轴、指标、趋势和不确定项。
...
- `pdf_metadata` / `pdf_extract_text`：读取 PDF 元数据、按 GPU 条件选择 Marker 或 MarkItDown，并生成相对路径图片素材。
- `paper_figure_analyze`：把 PDF 解析出的 figure/table 图片交给视觉后端，输出结构化证据。
- `bash`：运行本地脚本、clone GitHub、统计文件、检查环境；不要绕过 PDF 工具自行改变解析优先级。
