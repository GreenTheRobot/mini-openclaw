...
def test_permission_modes_have_distinct_write_and_execution_behavior(tmp_path: Path):
    ...
    assert check("write", args, tmp_path, mode="auto-local").verdict == "allow"
    assert check("bash", {"command": "python -V"}, tmp_path, mode="auto-safe").verdict == "confirm"
    assert check("bash", {"command": "bash snapshot.sh"}, tmp_path, mode="auto-local").verdict == "allow"
    assert check("web_fetch", {"url": "https://example.com/status"}, tmp_path, mode="auto-local").verdict == "allow"
